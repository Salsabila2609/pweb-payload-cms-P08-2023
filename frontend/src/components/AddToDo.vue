    ...
    <div class="flex flex-col gap-3">
      ...
      <div class="flex flex-col gap-1">
        ...
        <select v-model="inputCategory" class="px-3 py-2 bg-slate-100 rounded-md focus:outline-sky-800 cursor-pointer">
          <option v-for="Category in priorities" :key="Category.name" :value="Category.id" class="cursor-pointer">{{ Category.name }}</option>
        </select>
      </div>
    </div>
...
export default {
  data() {
    return {
      inputName: "",
      inputDeadline: "",
      inputCategory: "", // Category yang dipilih
      priorities: [], // Menyimpan data kategori prioritas
    };
  },
  methods: {
    addTodo() {
